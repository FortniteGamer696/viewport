...
local function getMouseMovement(dragToRotateViewportFrame)
	...
	if dragToRotateViewportFrame.MouseMode == "LockPosition" then
		movement = InputS:GetMouseDelta()
	elseif dragToRotateViewportFrame.MouseMode == "Default" then
		local mousePosition = InputS:GetMouseLocation()
...
function DragToRotateViewportFrame:SetModel(model)
	assertf(
		...
		CLASS_NAME, typeof(model), tostring(model)
	)
	assertf(
...
function DragToRotateViewportFrame:SetAngles(angles)
	assertf(
		...
		CLASS_NAME, typeof(angles), tostring(angles)
	)
...
function DragToRotateViewportFrame:Rotate(dPitch, dYaw)  
	...
	if self.PitchLimits then
		...
		fixedPitchLimits = self.RotateMode == "CameraRotates" and self.PitchLimits or NumberRange.new( -self.PitchLimits.Max, -self.PitchLimits.Min )
	end

	local fixedYawLimits
	if self.YawLimits then
		--Flip sign of min and max if RotateMdoe is CameraROtates
		fixedYawLimits = self.RotateMode == "CameraRotates" and self.YawLimits or NumberRange.new( -self.YawLimits.Max, -self.YawLimits.Min )
	end
	...
	if self.RotateMode == "CameraRotates" then 
		...
		limitedCFrame = constrainAngles( rotatedCFrame * CFrame.Angles(0, math.pi, 0), fixedPitchLimits, fixedYawLimits ) * CFrame.Angles(0, math.pi, 0)
	end
...
function DragToRotateViewportFrame:BeginDragging()
	...
	self.renderSteppedC = RunS.RenderStepped:Connect(function()
		if self.MouseMode == "LockCenter" then
			InputS.MouseBehavior = Enum.MouseBehavior.LockCurrentPosition
		else
			InputS.MouseBehavior = Enum.MouseBehavior.Default
		end
	end)

	self.inputChangedC = InputS.InputChanged:Connect(function(inputObject)
		if inputObject.UserInputType == Enum.UserInputType.MouseMovement then
			local delta = getMouseMovement(self)
